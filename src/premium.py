...
class Premium(commands.Cog):
    ...
    async def buy_role(self, interaction: discord.Interaction, role: str):
        ...
        if not guild_role:
            guild_role = await interaction.guild.create_role(
                ...
                permissions=discord.Permissions(**{perm: True for perm in role_data['permissions']})
            )
    ...
    @app_commands.command(name="premium_status", description="Проверить статус премиум подписки")
