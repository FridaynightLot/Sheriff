...
class PremiumFeatures(commands.Cog):
    ...
    @app_commands.command(
        name="mass_dm", description="Отправить сообщение всем участникам (ADMIN)"
    )
    @has_role("ADMIN")
    ...
    @app_commands.command(
        name="custom_role", description="Создать кастомную роль (PREMIUM)"
    )
    @has_role("PREMIUM")
    async def custom_role(
        self, interaction: discord.Interaction, name: str, color: str
    ):
        try:
    ...
    @app_commands.command(
        name="multiply_xp", description="Умножить опыт за сообщения (VIP)"
    )
    @has_role("VIP")
    ...
    async def multiply_xp(
        self, interaction: discord.Interaction, multiplier: int, duration: int
    ):
    ...
    @app_commands.command(
        name="custom_embed", description="Создать красивое сообщение (PREMIUM)"
    )
    @has_role("PREMIUM")
    async def custom_embed(
        self, interaction: discord.Interaction, title: str, description: str, color: str
    ):
        try:
            color_int = int(color.strip("#"), 16)
            embed = discord.Embed(
